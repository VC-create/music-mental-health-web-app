...
def services_piechart():
    ...
    #fig stands for figure which we created
...
def mh_issues_boxplot():
    ...
    df_long = pd.melt(df, id_vars=['Age'], value_vars=['Anxiety', 'Depression', 'Insomnia', 'OCD'], var_name='Condition', value_name='Level')
...
def effects_by_issue():
    ...
    fig.update_layout(height=500, width=500,
                    title_text="Music Effects for different Mental Health Issues", title=dict(font=dict(size=24)))
...
def age_condition_barchart(condition_filter):
    avg_condition=df.groupby('Age')[condition_filter].mean().reset_index()
    fig = px.bar(avg_condition, x='Age', y=condition_filter, labels={})
    fig = px.bar(avg_condition, x='Age', y=condition_filter, labels={'Age': 'Age',condition_filter: 'Average'}, 
...
def interactive_info():
    ...
    condition_filter = st.selectbox(label='Choose which mental health issue you want to view', 
                                    options=('Depression', 'Anxiety', 'Insomnia', 'OCD'), label_visibility='collapsed')
    age_condition_barchart(condition_filter)

#cache makes sure it won't reload the data over and over again 
#df reads the data and then gets rid of ages over 50
@st.cache_data
def get_data():
...
st.set_page_config(page_title = 'Music and Mental Health Dashboard', layout = 'wide', initial_sidebar_state= "expanded")


st.title("Music and Mental Health Dashboard")
df = get_data()

#everything inside the st.sidebar is about the sidebar
#the sidebar is called menu
#the user will select from the selectbox of 4 options
with st.sidebar:
    st.subheader("Menu")
...
#on the home page its displaying all the data
if selected == 'Home':
